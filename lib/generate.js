...
var generate = function (parser, exportName, wrapper) {
    ...
    return generate[parser.config.preprocessor](parser, exportName, wrapper);
};

var defaultJsWrapper = "(function () {\n%output%" +
    "if (typeof module !== 'undefined'&& typeof module.exports !== 'undefined') {\n" +
    "   module.exports = grammar;\n" +
    "} else {\n" +
    "   window.%export% = grammar;\n" +
    "}\n" +
    "})();\n";

generate.js = generate._default = generate.javascript = function (parser, exportName, wrapper) {
    if (!wrapper) {
        wrapper = defaultJsWrapper;
    }
    wrapper = ("// Generated automatically by nearley\n" + 
        "// http://github.com/Hardmath123/nearley\n" +
        wrapper).replace("%export%", exportName);
    
    var output = "function id(x) {return x[0]; }\n";
    output += parser.body.join('\n');
    output += "var grammar = {\n";
    output += "    ParserRules: " +
        serializeRules(parser.rules,
            generate.javascript.builtinPostprocessors) + "\n";
    output += "  , ParserStart: " + JSON.stringify(parser.start) + "\n";
    output += "};\n";
    return wrapper.replace("%output%", output);
};
generate.javascript.builtinPostprocessors = {
